...
class ParkingExit(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, debug_mode=False, criteria_enable=True,
        ...
        self._flow_distance = get_value_parameter(config, 'flow_distance', float, 100)
        logging.info(f"self._flow_distance==={self._flow_distance}")
        ...
        logging.info(f"config.trigger_points[0].location==={config.trigger_points[0].location}")
    ...
    def _initialize_actors(self, config):
        ...
        logging.info(f"front_points[0]==={front_points[0]}")
